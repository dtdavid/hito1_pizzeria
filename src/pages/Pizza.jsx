...
const { handleAgregar} = useContext(cartContext) // para el button añadir
const location = useLocation()
const [pizza, setPizza] = useState(location.state || null) 
//const {connect} = useContext(FetchContext) // acceder a las pizzas desde FetchContext
const {id} = useParams()
...
useEffect (() => {
    if(!pizza){
        const defaultId = id || "p001"
        fetch(`http://localhost:5000/api/pizzas/${defaultId}`)
        .then((res)=> res.json())
 ...
 // a veces no renderizaba nada!
  ...
  //const pizzaData = { ...connect, ...pizza } // para reducir el código
  return (
    <div className=" max-w-md rounded-lg overflow-hidden shadow-lg bg-white border border-gray-200 my-6">
        ...
        <footer className = "flex flex-col justify-around py-3 px-4 gap-5 text-justify border border-gray-200">
            ...
            <button onClick={() => { handleAgregar({id: pizza.id, name: pizza.name, price: pizza.price, img: pizza.img })
            }}
            className="bg-black text-white py-1 px-2 flex flex-row items-center justify-center gap-2 rounded-lg w-28">Añadir<GiShoppingCart className="mr-2" size = {20} /></button>
        </footer>
